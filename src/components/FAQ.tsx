...
const FAQ = () => {
  ...
  return (
    <div
      id="faq"
      className="relative h-full sm:w-[80%] py-16 bg-transparent backdrop-blur-sm border border-slate-600/30 rounded-lg my-16 mx-auto"
    >
        ...
        <Accordion type="single" collapsible className="space-y-4">
          {faqs.map((faq, index) => (
            <AccordionItem key={index} value={`faq-${index}`} className="">
              <AccordionTrigger className="px-4 py-2 text-lg hover:font-medium cursor-pointer">
                {faq.question}
              </AccordionTrigger>
